...
// src: CS offset, dst: CS offset, k: CS offset (>0)
// op2len<op1len: Modulo index operations (e.g., op2len=1, enabling array(op1)*scalar(op2[0])))
// k<0: array(dst)/scalar(-k)
void vecadd(sb2 op1, sb2 op2, sb2 dst, sb2 k) {
  ...
  if (pk) {
    for(index=0;index<dstlen;index++) {
      ks=(sb4)pk[index];
      t=(sb4)pop1[index]+(sb4)pop2[index%op2len];
      if (ks<0) pdst[index]=(sb2)(t/(-ks));
      else      pdst[index]=(sb2)(t*ks);
    };
  } else if (ks) {
    ...
    for(index=0;index<dstlen;index++) {
      pdst[index]=pop1[index]+pop2[index%op2len];
    };
  } 
