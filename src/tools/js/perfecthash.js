/* Perfect minimal hashing
   ...
   Usage (key: number, string):
   // word table ordered by hashtable, i.e., key == array index
   var words = [ "name", "name", .. ]
   var dict = { "name":key, .. } 
   var tables = create(dict)
   var key = lookup(tables[0],tables[1],name)
   // optional backward check if name can not be isin words!
   if (words[key]!=name) return NOTFOUND; else return key;
*/
function hash( d, str) {
  if(d == 0) { d = 0x811c9dc5; }
