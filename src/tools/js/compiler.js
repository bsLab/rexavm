...
new options:
  noIncude : Prevents including files
  noDefine : Prevents full evalaution and removing of #defines/#undef (only constants/macros by opts fields are used)
  strict   : Evaluate only conditionals with defined variables (i.e., #if X==v conditionals or so are evaluated only iff X is defined; otherwise preserved)
*/
...
Compiler.prototype.compileFile = function(file) {
	try {
	  var code = fs.readFileSync(_this.options.basePath + file, 'utf8')
		this.options.filename = file;
		var processor = new Processor(this, code);
		processor.run();
	} catch (err) {
			return this._error(`can't read file "${file}"`);
	};
};
...
Processor.prototype.run = function() {
	...
	function nextLine() {
		...
		_this.currentLineText=lines[_this.currentLine++]
		return _this.currentLineText;
	}
...
// Append currently parsed line to the result
Processor.prototype.addCurrentLine = function() {
  var line=this.currentLineText;
	this.result += line + this.options.newLine;
	this.emptyLines = 0;
};
...
Processor.prototype.parseNext = function() {
  ...
  // Do not handle include if noInclude option is set
  if (/^[ ]*#[ ]*include/.test(line) && this.options.noInclude)
		return this.addLine(this.addDefines(line));

  // Do not handle/remove defines if noDefine option is set
  if (/^[ ]*#[ ]*define/.test(line) && this.options.noDefine)
		return this.addLine(this.addDefines(line));

  // Do not handle/remove defines if noDefine option is set
  if (/^[ ]*#[ ]*undef/.test(line) && this.options.noDefine)
		return this.addLine(this.addDefines(line));
  ...
  name = text.substr(1).trimLeft().split(' ')[0];
  i = text.indexOf(name)+name.length;
  text = text.substr(i + 1).trimLeft();


	// Get the # directive
	var cmd = Directives[name];
	
// console.log('parseNext',name.trimLeft(),':',text.slice(0,10))
...
Compiler.prototype.createMacro = function(name, text) {
  var error=this.error||this._error;
	...
	if (end == -1)
		return error(`no closing parenthesis in the #define of macro ${name}`);
...
Compiler.prototype.addMacro = function(line, i, macro) {
  var self=this
	...
	for (var v, l = line.length; e < l; e++) {
		...
		if (v == "(") {
			...
			return { line: line, index: i }; // this.error(`there is no openning parenthesis for macro ${macro.name}`);
		}
	}
  ...
  if (macro.count==1 && macro.args[0]=='...') {
    // Special case: VARARGS!
	  // Replace macro variables with the given arguments
	  var str = macro.content[0].replace(/__VA_ARGS__/g,args.map(function (arg) {
	    return self.addDefines(arg)
	  }).join(','));
	...
	else if (args.length < macro.count)
...
Processor.prototype.conditionNext = function(end) {
	...
	// Count unexploited conditions, discard all lines up to end/else derictives!
  ...
  this.depth++;
	...
	expr = expr.replace(/defined\s*\(\s*([\s\S]+?)\s*\)/g, function(match, p1){
	  if (_this.options.strict) {
	    return _this.defines[p1] !== undefined? 'true' : match;
	  }
...
// console.log(expr)
	...
	try {
	  ...
	  if (this.options.strict) {
	    // ignore not defined variables
	    this.emitEnd[this.depth]=true;
	    this.addCurrentLine();
	    return;
	  }
...
createDirective("ifdef", function(text) {
	...
	if (this.defines[name] === undefined) {
	  if (this.options.strict) {
	    this.addCurrentLine();
	    this.emitEnd[this.depth]=true;
	    return; // continue parsing, keep ifdef incl. else endif
	  }
...
createDirective("ifndef", function(text) {
  ...
  if (this.options.strict) {
    this.addCurrentLine();
    this.emitEnd[this.depth]=true;
    return; // continue parsing, keep ifndef incl. else endif
  }
...
createDirective("else", function(expr, called) {
	...
	if (!called) {
	  if (this.options.strict) {
	    this.addCurrentLine();
	    return; // continue parsing, keep else endif
	  }
		return this.conditionEnd();
  }
...
createDirective("endif", function(expr, called) {
	...
	if (this.emitEnd[this.depth]) {
	  // except for preserved # blocks
	  this.emitEnd[this.depth]=false;
	  this.addCurrentLine();
  }
  if (this.depth) this.depth--;
});
...
createDirective("pragma", function(text) {
	...
	else if (text.toLowerCase().startsWith('ignore')) {
    // stop pre-processing until EOF
    this.ignore=true;
  }
	// Else: error
	else
