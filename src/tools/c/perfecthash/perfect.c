...
ub4  salt;
{
  FILE *f;
  char filename[100];
  sprintf(filename,"%shash.h",prefix);
  f = fopen(filename, "w");
  fprintf(f, "/* Automatically created on %s */\n",__DATE__);
  fprintf(f, "/* Perfect hash definitions */\n");
  fprintf(f, "#ifndef STANDARD\n");
  fprintf(f, "#include \"standard.h\"\n");
  fprintf(f, "#endif /* STANDARD */\n");
  fprintf(f, "#ifndef %sHASH\n",prefix);
  fprintf(f, "#define %sHASH\n",prefix);
  fprintf(f, "\n");
  if (blen > 0)
  {
#if 0
    ...
    fprintf(f, "#define %sHASHLEN 0x%lx  /* length of hash mapping table */\n",
	    prefix,blen);
  }
  fprintf(f, "#define %sHASHNKEYS %ld  /* How many keys were hashed */\n",
          prefix,nkeys);
  fprintf(f, "#define %sHASHRANGE %ld  /* Range any input might map to */\n",
          prefix,smax);
  fprintf(f, "#define %sHASHSALT 0x%.8lx /* internal, initialize normal hash */\n",
          prefix,salt*0x9e3779b9);
  fprintf(f, "\n");
  fprintf(f, "ub4 %shash(char *key, int len);\n",prefix);
  fprintf(f, "\n");
  fprintf(f, "#endif  /* %sHASH */\n",prefix);
  fprintf(f, "\n");
  fclose(f);
}
...
hashform *form;                                           /* user directives */
{
  ...
  FILE *f;
  char filename[100];
  sprintf(filename,"%shash.c",prefix);
  f = fopen(filename, "w");
  fprintf(f, "/* Automatically created on %s */\n",__DATE__);
...
"0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx, 0x%.4lx,\n",
  ...
  if (blen > 0)
  {
    ...
    if (smax <= UB1MAXVAL+1 || blen >= USE_SCRAMBLE)
      fprintf(f, "static ub1 tab[] = {\n",prefix);
    else
      fprintf(f, "static ub2 tab[] = {\n",prefix);
  ...
  case HEX_HM:
  case DECIMAL_HM:
    fprintf(f, "ub4 %shash(val)\n",prefix);
    fprintf(f, "ub4 val;\n");
    break;
  case AB_HM:
  case ABDEC_HM:
    fprintf(f, "ub4 %shash(a,b)\n",prefix);
    fprintf(f, "ub4 a;\n");
    fprintf(f, "ub4 b;\n");
    break;
  }
